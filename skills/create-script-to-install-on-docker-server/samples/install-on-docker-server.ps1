...
.DESCRIPTION
  Sample for devmin/scripts/docker/<repo_name>/install-on-docker-server.ps1.
  Reads install-on-docker-server.yaml — no CLI -- flags except optional -Stop.
...
function Show-Help {
    Write-Host @"
install-on-docker-server.ps1 — remote Docker first-time install (YAML-only)

USAGE:
  .\.armin\scripts\docker\install-on-docker-server.ps1

CONFIG:
  Sibling file: install-on-docker-server.yaml
...
if ($args.Count -gt 0) {
    Write-Fail 'Unknown CLI arguments. Use optional -Stop or edit install-on-docker-server.yaml.'
    Show-Help
    exit 1
}
...
try {
    ...
    if (Test-Placeholder $sshValue) {
        throw 'ssh still has placeholders. Fill install-on-docker-server.yaml before server deploy.'
    }
    ...
    if ($buildImageOn -eq 'server') {
        ...
        $yamlLocal = Join-Path $DeployDir 'install-on-docker-server.yaml'
        if (-not (Test-Path -LiteralPath $yamlLocal)) { throw "Sync source not found: $yamlLocal" }
        Write-Step 'Sync install-on-docker-server.yaml'
        Copy-ToRemote -Target $target -LocalPath $yamlLocal -RemotePath "$volumeDir/install-on-docker-server.yaml"
    }
    ...
    Write-Ok "Fresh install complete at $volumeDir on $($target.LogTarget)"
}
catch {
