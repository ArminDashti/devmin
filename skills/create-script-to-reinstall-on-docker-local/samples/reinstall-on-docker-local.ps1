...
.DESCRIPTION
  Sample for devmin/scripts/docker/<repo_name>/reinstall-on-docker-local.ps1.
  Reads reinstall-on-docker-local.yaml — no CLI -- flags.
...
$ConfigPath = Join-Path $DeployDir 'reinstall-on-docker-local.yaml'
...
function Show-Help {
    Write-Host @"
reinstall-on-docker-local.ps1 — local Docker reinstall (remove completely then install) (YAML-only)

USAGE:
  .\.armin\scripts\docker\reinstall-on-docker-local.ps1

CONFIG:
  Sibling file: reinstall-on-docker-local.yaml
...
if ($args.Count -gt 0) {
    Write-Fail 'This script accepts no CLI arguments. Edit reinstall-on-docker-local.yaml instead.'
    Show-Help
    exit 1
}
...
try {
    ...
    Write-Step "Stack=$stackName image=$imageTag network=$network publish_port='$publishPort' internal_port='$internalPort' delete_volume=$deleteVolume delete_image=$deleteImage (reinstall wipe)"
    ...
    Write-Ok 'Reinstall complete (removed then installed)'
}
catch {
