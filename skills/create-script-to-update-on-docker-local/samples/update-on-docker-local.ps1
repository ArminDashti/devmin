...
.DESCRIPTION
  Sample for devmin/scripts/docker/<repo_name>/update-on-docker-local.ps1.
  Reads update-on-docker-local.yaml — no CLI -- flags.
...
function Show-Help {
    Write-Host @"
update-on-docker-local.ps1 — local Docker update/redeploy (YAML-only)

USAGE:
  .\.armin\scripts\docker\update-on-docker-local.ps1

CONFIG:
  Sibling file: update-on-docker-local.yaml
...
if ($args.Count -gt 0) {
    Write-Fail 'This script accepts no CLI arguments. Edit update-on-docker-local.yaml instead.'
    Show-Help
    exit 1
}
...
try {
    ...
    if ($deleteImage) {
        ...
        # Missing image must not abort (StrictMode / native stderr).
