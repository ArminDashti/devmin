...
.DESCRIPTION
  Sample for devmin/scripts/docker/<repo_name>/reinstall-on-docker-server.ps1.
  Reads reinstall-on-docker-server.yaml — no CLI -- flags except optional -Stop.
...
$ConfigPath = Join-Path $DeployDir 'reinstall-on-docker-server.yaml'
...
function Show-Help {
    Write-Host @"
reinstall-on-docker-server.ps1 — remote Docker reinstall (remove completely then install) (YAML-only)

USAGE:
  .\.armin\scripts\docker\reinstall-on-docker-server.ps1

CONFIG:
  Sibling file: reinstall-on-docker-server.yaml
...
if ($args.Count -gt 0) {
    Write-Fail 'Unknown CLI arguments. Use optional -Stop or edit reinstall-on-docker-server.yaml.'
    Show-Help
    exit 1
}
...
try {
    ...
    # Install skill contract: always wipe this stack’s containers, volumes/DB, and image.
    $deleteVolume = $true
    $deleteImage = $true
    ...
    if (Test-Placeholder $sshValue) {
        throw 'ssh still has placeholders. Fill reinstall-on-docker-server.yaml before server deploy.'
    }
    ...
    Write-Step "Stack=$stackName image=$imageTag build_image_on=$buildImageOn volume_dir=$volumeDir publish_port='$publishPort' internal_port='$internalPort' (fresh install wipe)"
    ...
    if ($buildImageOn -eq 'server') {
        ...
        $yamlLocal = Join-Path $DeployDir 'reinstall-on-docker-server.yaml'
        if (-not (Test-Path -LiteralPath $yamlLocal)) { throw "Sync source not found: $yamlLocal" }
        Write-Step 'Sync reinstall-on-docker-server.yaml'
        Copy-ToRemote -Target $target -LocalPath $yamlLocal -RemotePath "$volumeDir/reinstall-on-docker-server.yaml"
    }
    ...
    Write-Step 'Remote compose up -d (fresh)'
    ...
    Write-Ok "Reinstall complete at $volumeDir on $($target.LogTarget)"
}
catch {
