...
.DESCRIPTION
  Sample for devmin/scripts/docker/<repo_name>/update-on-docker-server.ps1.
  Reads update-on-docker-server.yaml — no CLI -- flags except optional -Stop.
...
function Show-Help {
    Write-Host @"
update-on-docker-server.ps1 — remote Docker update/redeploy (YAML-only)

USAGE:
  .\.armin\scripts\docker\update-on-docker-server.ps1

CONFIG:
  Sibling file: update-on-docker-server.yaml
...
if ($args.Count -gt 0) {
    Write-Fail 'Unknown CLI arguments. Use optional -Stop or edit update-on-docker-server.yaml.'
    Show-Help
    exit 1
}
...
try {
    ...
    if (Test-Placeholder $sshValue) {
        throw 'ssh still has placeholders. Fill update-on-docker-server.yaml before server deploy.'
    }
