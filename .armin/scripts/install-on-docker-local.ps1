...
<#
.SYNOPSIS
  Install devmin on local Docker using docker-compose.prod.yml (Postgres + API + WebUI).
#>
[CmdletBinding()]
param()
...
function Ensure-Docker {
    docker version *> $null
    if ($LASTEXITCODE -ne 0) { throw 'Docker CLI is not available. Start Docker Desktop.' }
}
...
try {
    ...
    Write-Step "Local Docker install stack=$stackName"
    Ensure-Docker
    Ensure-Network $network
    docker compose -p $stackName -f $composePath --project-directory $ComposeProjectDir up -d --build
    if ($LASTEXITCODE -ne 0) { throw 'docker compose up failed' }
    Write-Ok "Local Docker install complete. WebUI: http://127.0.0.1:5195/apps"
}
catch {
