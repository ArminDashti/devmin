...
- Prefer existing plugin skills (e.g. `run-this-app-on-local-machine-hot-reload`) over creating duplicate skills when one already covers the task.
...
- Four deployment channels: hot-reload, local Docker, server Docker, and bare server — detail pages expose enable/disable or install/uninstall/update/reinstall per channel.
...
- Deploy script convention: `.armin/docker-scripts/run-on-docker-local.ps1`, `run-on-docker-server.ps1`, and `.armin/server-scripts/run-on-server.ps1` with `-Action Install|Uninstall|Update|Reinstall`.
